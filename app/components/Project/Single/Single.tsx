...
export default function SingleProject({ title, links, thumbnail, shortDescription, techList, animationDelay = 0, reverse = false }: Props) {
  return (
    <div className={`single-project ${reverse ? 'reverse' : ''}`} data-aos='fade-up' data-aos-delay={animationDelay}>
      <div className='title md:hidden'>{title}</div>
      ...
      <div className='project-content'>
        <div className='title hidden md:block'>{title}</div>
